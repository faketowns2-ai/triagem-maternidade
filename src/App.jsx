...
const buildWhatsAppLink = (data, tipo) => {
  ...
  if (tipo === "gravida") {
    msg =
      `Olá! Vim pelo formulário de triagem.\n\n` +
...
// Etapa 1: salva nome + whatsapp imediatamente, retorna rowId
const sendLeadEtapa1 = async (payload) => {
  try {
...
// Etapa 2: atualiza CPF na linha já existente
const updateCpfEtapa2 = async (rowId, cpf) => {
  try {
    await fetch(GOOGLE_SCRIPT_URL, {
      method: "POST",
      headers: { "Content-Type": "text/plain" },
      body: JSON.stringify({ action: "updateCpf", rowId, cpf }),
    });
  } catch (_) {}
};
...
export default function Triagem() {
  ...
  const [salvandoEtapa1, setSalvandoEtapa1] = useState(false);
  ...
  // Avança da etapa whatsapp: salva imediatamente na planilha
  const handleAvancarContato = async () => {
    setSalvandoEtapa1(true);
    fbq("track", "AddToCart", { content_name: "Lead Etapa 1 - WhatsApp" });
    const id = await sendLeadEtapa1({
      nome: data.nome,
      tipo: data.tipo,
    ...
    setRowId(id);
    setSalvandoEtapa1(false);
    goStep("cpf");
  };

  // Finaliza: atualiza CPF e vai para resultado
  const handleVerResultado = async () => {
    if (rowId && data.cpf) {
      await updateCpfEtapa2(rowId, data.cpf);
    }
    fbq("track", "Lead", {
  ...
  const telas = {
      ...
      <div style={S.card}>
        ...
        <div style={S.cardBody}>
          ...
          <div style={S.socialProof}>
            <span style={S.socialProofIcon}>✅</span>
            <span style={S.socialProofText}>Mais de 1.200 mães já verificaram seu direito</span>
          </div>
          ...
          <button style={S.btnPrimary} onClick={() => { fbq("track", "InitiateCheckout", { content_name: "Triagem" }); goStep("nome"); }}>
    ...
    nome: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={1} total={5} label="Identificação" />
        ...
        <div style={S.cardBody}>
          ...
          />
          <button
            style={data.nome.trim().length >= 3 ? S.btnPrimary : S.btnDisabled}
    ...
    tipo: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={2} total={5} label="Situação" />
        ...
        <div style={S.cardBody}>
          {[
            ...
            <button
              key={op.val}
              style={{ ...S.optionBtn, ...(data.tipo === op.val ? S.optionBtnActive : {}) }}
    ...
    gestacao: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={3} total={5} label="Gestação" />
        ...
        <div style={S.cardBody}>
          <div style={S.mesGrid}>
            {MESES.map((m) => (
              <button
                key={m}
                style={{ ...S.mesBtn, ...(data.mesGestacao === m ? S.mesBtnActive : {}) }}
    ...
    parto: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={3} total={5} label="Parto" />
        ...
        <div style={S.cardBody}>
          ...
          <input
            ...
            inputMode="numeric"
            value={data.dataParto}
            onChange={(e) => set("dataParto", formatDate(e.target.value))}
          />
          ...
          <div style={S.radioGroup}>
            {["Sim", "Não"].map((op) => (
              <button
                key={op}
                style={{ ...S.radioBtn, ...(data.trabalhouCLT === op ? S.radioBtnActive : {}) }}
                onClick={() => set("trabalhouCLT", op)}
              >
                {op}
              </button>
            ))}
          ...
          <button
            style={data.dataParto.length >= 10 && data.trabalhouCLT ? S.btnPrimary : S.btnDisabled}
            disabled={!data.dataParto || data.dataParto.length < 10 || !data.trabalhouCLT}
            onClick={() => goStep("contato")}
          >
            Continuar
          </button>
          <BackBtn onClick={() => goStep("tipo")} />
    ...
    contato: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={4} total={5} label="WhatsApp" />
        ...
        <div style={S.cardBody}>
          ...
          <input
            style={{
              ...S.input,
              ...(data.contato.length === 15 ? validarWhatsApp(data.contato) ? S.inputValid : S.inputError : {}),
            }}
          ...
          />
          {data.contato.length === 15 && !validarWhatsApp(data.contato) && (
            <p style={S.errorMsg}>Número inválido. Informe um celular brasileiro com DDD.</p>
          )}
          {data.contato.length === 15 && validarWhatsApp(data.contato) && (
            <p style={S.successMsg}>✓ Número válido!</p>
          )}
          <button
            style={validarWhatsApp(data.contato) && !salvandoEtapa1 ? S.btnPrimary : S.btnDisabled}
            disabled={!validarWhatsApp(data.contato) || salvandoEtapa1}
            onClick={handleAvancarContato}
          >
            {salvandoEtapa1 ? "Salvando..." : "Continuar →"}
          </button>
    ...
    cpf: (
      <div style={S.card}>
        <div style={S.stepHeader}>
          <StepPill current={5} total={5} label="CPF" />
          <h2 style={S.stepTitle}>Quase lá! Informe seu CPF</h2>
          <p style={S.stepSub}>Necessário para consultar seu benefício no INSS.</p>
        </div>
        <div style={S.cardBody}>
          <div style={S.securityBadge}>
            <span style={S.securityIcon}>🔒</span>
            <span style={S.securityText}>Seus dados são criptografados e usados apenas para consulta do benefício. Não compartilhamos com terceiros.</span>
          </div>
          <label style={S.label}>CPF</label>
          <input
            style={{
              ...S.input,
              ...(data.cpf.length === 14 ? validarCPF(data.cpf) ? S.inputValid : S.inputError : {}),
            }}
          ...
          />
          {data.cpf.length === 14 && !validarCPF(data.cpf) && (
            <p style={S.errorMsg}>CPF inválido. Verifique os números e tente novamente.</p>
          )}
          {data.cpf.length === 14 && validarCPF(data.cpf) && (
            <p style={S.successMsg}>✓ CPF válido!</p>
          )}
          <button
            style={validarCPF(data.cpf) ? S.btnPrimary : S.btnDisabled}
            disabled={!validarCPF(data.cpf)}
          ...
          <BackBtn onClick={() => goStep("contato")} />
        </div>
      </div>
    ),
    ...
    resultado: (
      <div style={S.card}>
        ...
        <div style={S.cardBody}>
          ...
          <p style={S.resultSub}>
            Ótimo, {data.nome.split(" ")[0]}! Suas informações foram recebidas.
            Nossa equipe vai analisar seu caso e entrar em contato em breve.
          </p>
          <div style={S.resultBox}>
            <p style={S.resultBoxText}>
              Clique abaixo para falar diretamente com nossa equipe e agilizar seu atendimento:
            </p>
          </div>
          <a
            href={buildWhatsAppLink(data, data.tipo)}
            target="_blank"
          ...
          <button style={S.btnGhost} onClick={() => {
            setData({ nome:"",cpf:"",tipo:"",mesGestacao:"",dataParto:"",trabalhouCLT:"",contato:"" });
            setRowId(null);
            goStep("inicio");
          }}>
            Nova triagem
...
function StepPill({ current, total, label }) {
  return (
    <div style={S.stepPillWrap}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <div style={S.stepPill}>{label || `Passo ${current}`}</div>
        <span style={S.stepCounter}>{current} de {total}</span>
      </div>
...
function BackBtn({ onClick }) {
  return (
    <button style={S.backBtn} onClick={onClick}>← Voltar</button>
  );
}
...
const S = {
  ...
  bg: {
    ...
    background: `radial-gradient(ellipse at 20% 10%, ${C.terra3}22 0%, transparent 50%),
                 radial-gradient(ellipse at 80% 80%, ${C.dourado}18 0%, transparent 50%),
                 linear-gradient(160deg, ${C.bege} 0%, ${C.bege2} 100%)`,
  },
  pageWrap: {
  ...
  card: {
    flex: 1,
    background: C.white,
    boxShadow: "0 4px 40px rgba(61,43,31,0.10)",
    display: "flex", flexDirection: "column",
    minHeight: "100vh",
  },
  cardBody: {
  ...
  bannerOverlay: {
    position: "absolute", inset: 0,
    background: "linear-gradient(to bottom, transparent 50%, rgba(61,43,31,0.3) 100%)",
  },
  bannerPlaceholder: {
    width: "100%", height: "100%",
    display: "flex", flexDirection: "column",
    alignItems: "center", justifyContent: "center",
    background: `linear-gradient(135deg, ${C.bege2} 0%, ${C.terra3}33 100%)`,
    gap: 8,
    border: `2px dashed ${C.terra3}66`,
  },
  ...
  tagline: {
    ...
    color: C.terracota, fontWeight: 600,
  },
  title: {
  ...
  divider: {
    height: 1,
    background: `linear-gradient(to right, ${C.dourado}60, transparent)`,
    margin: "4px 0",
  },
  socialProof: {
    display: "flex", alignItems: "center", gap: 8,
    background: `${C.terra3}15`,
    border: `1px solid ${C.terra3}40`,
    borderRadius: 8, padding: "10px 14px",
  },
  socialProofIcon: { fontSize: 16, flexShrink: 0 },
  socialProofText: {
  ...
  stepHeader: {
    background: `linear-gradient(135deg, ${C.terra3}22 0%, ${C.dourado}11 100%)`,
  ...
  progressBar: {
    ...
    background: C.bege2, overflow: "hidden",
    marginTop: 8,
  },
  progressFill: {
    height: "100%",
    background: `linear-gradient(to right, ${C.terracota}, ${C.dourado})`,
    borderRadius: 4,
  ...
  stepSub: {
    fontSize: 14, color: C.textMuted,
    fontFamily: "sans-serif", margin: 0, lineHeight: 1.5,
  },
  label: {
  ...
  input: {
    ...
    border: `1.5px solid ${C.bege2}`,
    borderRadius: 10, outline: "none",
    color: C.text, background: C.bege3,
  ...
  securityIcon: { fontSize: 18, flexShrink: 0, marginTop: 1 },
  securityText: {
    fontSize: 12, color: "#1a4a7a",
  ...
  mesBtnActive: {
    background: `${C.terra3}22`,
    borderColor: C.terracota,
    color: C.terracota, fontWeight: 700,
  },
  ...
  btnPrimary: {
    ...
    background: `linear-gradient(135deg, ${C.terracota} 0%, ${C.terra2} 100%)`,
    color: C.white,
    border: "none", borderRadius: 12,
    fontSize: 16, fontWeight: 700,
    fontFamily: "sans-serif", cursor: "pointer",
    letterSpacing: "0.02em",
    boxShadow: `0 4px 16px ${C.terracota}44`,
  ...
  resultBox: {
    background: `linear-gradient(135deg, ${C.bege2}, ${C.terra3}22)`,
    borderRadius: 12, padding: "16px",
    border: `1px solid ${C.terra3}44`,
  },
  resultBoxText: {
